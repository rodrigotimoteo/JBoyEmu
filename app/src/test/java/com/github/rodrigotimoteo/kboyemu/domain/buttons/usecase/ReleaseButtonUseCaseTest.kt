...
/**
 * Unit tests for [ReleaseButtonUseCase]
 *
 * @author rodrigotimoteo
 */
class ReleaseButtonUseCaseTest {

    /** Mock for [KBoyEmulator] */
    private val emulatorMock: KBoyEmulator = mockk(relaxed = true)

    /** Instance of the use case being tested */
    private val sut: ReleaseButtonUseCase = ReleaseButtonUseCase(emulatorMock)

    @EnumSource(Button::class)
    @ParameterizedTest
    fun `when invoking with a button then button release is sent to emulator`(button: Button) {
        sut(button)

        verify(exactly = 1) { emulatorMock.release(button) }
    }
}
